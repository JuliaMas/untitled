...
import org.example.exceptions.homeWork_3.moduls.NumberException;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class CheckUser implements CheckUserInt {
    List<String> abc;
    List<Integer> num;

    public CheckUser() {
        this.abc = new ArrayList<>(Arrays.asList("а", "б", "в", "г", "д", "е", "ё", "ж", "з", "и", "й", "к",
    ...
    public String checkName(String name) {
        ...
        for (int i = 0; i < str.length(); i++) {
            if (!abc.contains(String.valueOf(str.charAt(i)).toLowerCase())) {
    ...
    public String checkNumber(String number) {
        ...
        if (str.length() != 11) {
            throw new RuntimeException("Неверный формат! Вводите 11 цифр(89xxxxxxxxx)! А не это -> " + str);
        }
        for (int i = 0; i < str.length(); i++) {
            try {
